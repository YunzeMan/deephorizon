...
def preprocess(img, caffe_size):
    # scale to [0, 255]
    img = (255*img).astype(np.float32, copy=False)

    # channel swap (RGB -> BGR)
    img = img[:, :, [2, 1, 0]]

    # make caffe size
    img = caffe.io.resize_image(img, caffe_size)

    # subtract mean
    img = img - np.asarray([104, 117, 123]).reshape((1, 1, 3))

    # make channels x height x width
    img = img.swapaxes(0, 2).swapaxes(1, 2)

    input_caffe = img.reshape((1,)+img.shape)
...
def compute_horizon(slope_dist, offset_dist, caffe_sz, sz, crop_info, bin_edges):
    ...
    left_h, right_h = extrap_horizon(left_tmp, right_tmp, sz[1])

    return [np.squeeze(left_h), np.squeeze(right_h)]
